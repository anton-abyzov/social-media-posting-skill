...
## Brand & Logo Fidelity (Non-Negotiable)

Whenever a post depicts recognizable brands, products, companies, or people, the visuals MUST use the real, official logos / icons / product shots. Do NOT let Nano Banana, Veo, or any other generator invent stylized blob-shapes that merely *resemble* a brand. A hallucinated logo is a hard regeneration trigger, not an acceptable artifact.

**Why this rule exists:** Past runs produced "neon glow" abstract shapes the model labeled as "OpenAI", "Azure", "xAI", "AWS" — they were not the real logos and looked off-brand. Real users instantly notice. Brand fidelity is what separates a credible editorial render from AI slop.

### a) Research Before Generating

Before any Nano Banana / Veo / image-gen call that should depict real brands, locate the *actual* official logos. Try sources in this order:

1. **Local user assets** — `~/Pictures`, `~/Desktop`, `~/Documents/logos`, the project's `assets/` folder, and any `.claude/brand/` directories. Many campaigns reuse pre-vetted brand kits.
2. **The user's Obsidian vault** — `~/Projects/Obsidian/personal-docs/` may already contain logos under `003 Resources/` or attached to brand notes. Search for the brand name.
3. **Wikimedia Commons direct URLs** — e.g., `https://upload.wikimedia.org/wikipedia/commons/thumb/4/44/Microsoft_logo.svg/1280px-Microsoft_logo.svg.png`. Authoritative, public-domain, and Kie.ai's servers can fetch them.
4. **Brand portals** — `microsoft.com/brand`, `openai.com/brand`, `aws.amazon.com/architecture/icons`, etc. These are the source of truth.
5. **Fallback:** `logos-world.net/wp-content/uploads/...` — usually real PNGs and unblocked.

**Avoid:**
- `cdn.brandfetch.io` direct paths — they require API auth and return HTML, not images.
- Wikipedia `thumb/` paths that return HTML — always verify the downloaded file with `file <path>` and confirm it reports `PNG image` (or `SVG`/`JPEG`) before using.
- Random Google Image search results — they are often watermarked, low-res, or incorrect.

After downloading, run `file <path>` to confirm it's a real image, and `identify <path>` (ImageMagick) or `sips -g pixelWidth -g pixelHeight <path>` to confirm dimensions are sane (>= 512px on the long edge for clean rendering).

### b) Pass Real Logos as `image_input` to Nano Banana Pro

The `nano-banana-pro` model accepts up to **14 image URLs** in the `input.image_input` array. Use this — do NOT rely on the prompt alone to summon a logo from latent space.

Workflow:
1. Collect the verified logo URLs (or local files).
2. If a logo is local, upload it to a public host first so Kie.ai can fetch it. Options in priority order:
   - `postiz upload <file>` (returns a hosted URL on `postiz.easychamp.com`)
   - Imgur anonymous upload via API
   - Any HTTPS CDN reachable from Kie.ai's servers
3. Pass every logo URL in `input.image_input`. Reference them by name in the prompt: *"the OpenAI logo (image 1) on the left side of the frame, the Microsoft Azure logo (image 2) on the right"*.

### c) Prompt Language

Always include this clause verbatim (or a near-equivalent) in the prompt for any brand-bearing generation:

> *"Use the supplied reference images verbatim — do not stylize the logos, do not invent shapes, render them photorealistically as real brand assets. Logos must be pixel-accurate to the references."*

### d) Style Direction

Default visual style for brand-bearing posts: **photorealistic editorial 3D render with real logos rendered cleanly**.

**Use these words:**
- "photorealistic"
- "studio product render"
- "real brand assets"
- "cinematic but accurate"
- "editorial photography"
- "physically-based materials, accurate reflections"

**Avoid these words** (they produce hallucinated blob-shapes that *look like* logos but are not):
- "non-Euclidean"
- "stylized" (unless the entire piece is meant to be illustrative and brand-free)
- "abstract"
- "ethereal logos"
- "glowing neon logos" (the glow takes over and the logo deforms)
- "logo reimagined as..." (model will reimagine, that's the problem)

### e) Validate Before Publishing

After generation, do a hard check:
1. Visually confirm each logo matches the reference. Zoom in on each one.
2. If a vision model is available (Claude with image input, Gemini, GPT-4V), ask it: *"Are the logos in this image actually accurate to the supplied references? List any that look wrong, distorted, or invented."*
3. If anything is off — wrong colors, wrong typography, missing details, invented shapes — **regenerate**. Do not ship and do not "just go with it".
4. Only after validation passes, hand off to the human for approval per Core Rule #1.

### Verified Logo URL Reference Table

These URLs have been confirmed to return real PNG/SVG image data and are reachable from Kie.ai's servers. **Append to this table every time the workflow encounters a new brand whose logo had to be researched.** The skill is supposed to grow this table over time.

| Brand | URL | Format |
|-------|-----|--------|
| Microsoft | `https://upload.wikimedia.org/wikipedia/commons/thumb/4/44/Microsoft_logo.svg/1280px-Microsoft_logo.svg.png` | PNG |
| Microsoft Azure | `https://upload.wikimedia.org/wikipedia/commons/thumb/a/a8/Microsoft_Azure_Logo.svg/1280px-Microsoft_Azure_Logo.svg.png` | PNG |
...
- Mandate real-logo research and `image_input` passthrough for Nano Banana Pro.
- Added verified logo URL reference table for major AI / cloud brands.
- Default visual style flipped from stylized to photorealistic editorial.
